...
form.addEventListener('submit', applySolver);


function applySolver(event) {
    ...
    console.log(pattern)
    let guesses = document.getElementById("guessedLetters").value
    console.log(guesses)
    let results = solve(hint, pattern, guesses)
    let resultElement = document.getElementById("answer")
    if (results.length === 0) resultElement.innerText = "Answer not found"
    else if (results.length === 1) resultElement.innerText = "The Answer is: " + results.toString()
    else resultElement.innerText = "Options: " + results.toString()
    document.getElementById("answer").innerText = solve(hint, pattern, guesses).toString()
}

function hinted(hint) {
    let words = []
    for (let index in json["words"]) {
        let values = json["words"][index]
        if (values["hint"].toLowerCase() === hint.toLowerCase()) {
            words.push(values["word"].toLowerCase())
        }
    }
    return words
}
...
function matches(pattern, answer, guessedLetters) {
    ...
    for (let letter in guessedLetters) {
        if(pattern.includes(letter)) continue
        if (answer.includes(letter)) return false
    }

    // check match
    for (let i = 0; i < pattern.length; i++) {
        let pat = pattern.charAt(i)
        let ans = answer.charAt(i)

        if (pat !== ans) {
            // console.log(pat + ", " + ans)
            if (pat === "_" && ans !== " " && !guessedLetters.includes(ans) && !pattern.includes(ans)) continue
            return false
        }
    }
    return true
...
function solve(hint, pattern, guessedLetters) {
    let words = hinted(hint)
    console.log(words)
    console.log(words.length)

    let matchingWords = words.filter(
        function (word) {return matches(pattern, word, guessedLetters)}
    )
